...
function selectRandom(cardArr) {
  ...
  if (cardArr.length === 1) {
    ...
    // math.random creates a random number that is 0 <= number < 1
    // math.floor rounds down that number to an integer
    // both return numbers that act as the random index to return a value from the array
    random = cardArr[Math.floor(Math.random() * cardArr.length)];
  } else {
  }
  return random;
}
